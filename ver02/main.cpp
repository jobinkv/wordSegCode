...
int main (int argc, char * const argv[]) 
{
	...
	srand(4);// chage the number will result defferent trainFiles, validationFiles and testFiles. 
	...
	if(TEST_MODEL)	{
		...
		stringstream optloc;
		optloc.str("");
		optloc << "mkdir -p " +validOpImg+"stage1/";
		system(ss.str().c_str());
		for (unsigned int i = 0;i < validationFiles.size();i++) {
			...
			outname.str("");
			outname <<optloc<<validationFiles[i]<<".words.txt";
			int num_labels = 2;
			int lambada=2*255;
