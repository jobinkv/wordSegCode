...
#include "rectprior.h"
void HandDetector::loadMaskFilenames(vector<string> msk_prefix)
{
	...
	_filenames.reserve(msk_prefix.size()); 
	copy(msk_prefix.begin(),msk_prefix.end(),back_inserter(_filenames));
	//getdir(msk_prefix,_filenames);
}


void HandDetector::trainModels(string basename, string img_prefix,string msk_prefix,string model_prefix,string globfeat_prefix, string feature_set, int max_models, int width, int channel)
{
	...
	for (unsigned int i = 0;i <_filenames.size();i++) 
	{
		...
		//cout<<"_filenames.size() = "<<ss<<endl;
		//ss << msk_prefix << f << ".jpg";
		cout <<ss.str()<< endl;
		Mat mask_img1 = imread(ss.str(),1);
		Mat gtSplit[3];
		split(mask_img1, gtSplit);
		Mat mask_img = gtSplit[channel].clone();
		//namedWindow( "Display window1", WINDOW_NORMAL );// Create a window for display.
		//imshow( "Display window1", mask_img ); 
		//waitKey(0); 	
		...
		ss.str("");
		ss << img_prefix <<_filenames[i];
		//ss << img_prefix << f+1 << ".png"; // one based?
		...
		if(VISUALIZE)
		{
			...
			// imshow("blend",dsp);
			// waitKey(0);
            // lab.convertTo(Show,CV_8UC1);
    		namedWindow( "Display window", WINDOW_NORMAL );// Create a window for display.
    		imshow( "Display window", dsp ); 
		    waitKey(0);	
		}
		...
		ss.str("");
		ss << globfeat_prefix << "hsv_histogram_" << i << ".xml";
		...
		*/
		...
		//cout<<"size 1 = "<<color_img.size()<<endl;
...
void HandDetector::test(Mat &img, Mat &dsp, int num_models, int step_size)
{
   ...
   // _img_height = 1293;
    //cout<<"_img_height = "<<_img_height<<endl;
    //cout<<"img.cols = "<<img.cols<<endl;
	_img_size   = Size(_img_width,_img_height);
	//cout<<"tttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt "<<endl;	
	//resize(img,img,_img_size);
	

	//cout<<"_img_size = "<<_img_size<<endl;
	Mat hist;
	computeColorHist_HSV(img,hist);									// extract hist
	
	_searchtree.knnSearch(hist, _indices, _dists, _knn, flann::SearchParams(4));			// probe search
	
	_extractor.work(img,_descriptors,step_size,&_kp);						// every 3rd pixel
	//cout<<"_descriptors = "<<_descriptors.size()<<endl;
	//if(!_response_avg.data) 
	_response_avg = Mat::zeros(_descriptors.rows,1,CV_32FC1); 
	//else _response_avg *= 0;
		//cout<<"gggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggg "<<endl;
	float norm = 0;
	for(int i=0;i<num_models;i++)
	{
		int idx = _indices[i];
		_classifier[idx].predict(_descriptors,_response_vec);		// run classifier
		//cout<<"11111111111  = "<<_response_vec.size()<<endl;
       // cout<<"11111111111  = "<<float(pow(0.9f,(float)i))<<endl;
		_response_avg += _response_vec*float(pow(0.9f,(float)i));
        //cout<<"222222 "<<endl;
		norm += float(pow(0.9f,(float)i));
	}
		//cout<<"ddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd "<<endl;
	_response_avg /= norm;
